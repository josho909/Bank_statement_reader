...
#                 Return the second table between Deposits Balance\n and \nTransaction Total
                ...
                trans = match[table_position.iloc[0]]
                for i, row in text_key.iterrows():
                    if str(row.iloc[1]) == 'nan':
                        trans = re.sub(str(row.iloc[0]), '', trans)
                    elif str(row.iloc[1]) == 'Y':
                        trans = re.sub(str(row.iloc[0]), '', trans, flags=re.MULTILINE)
                    else:
                        trans = re.sub(str(row.iloc[0]), str(row.iloc[1]), trans)
...
for account_type, table_position in account_table.iterrows():
    print(f'Reading of {account_type} table in progress...')
